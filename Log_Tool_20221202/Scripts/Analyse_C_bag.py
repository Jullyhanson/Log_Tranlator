...
class C_bag_Analyse():
    def __init__(self,file_c_bag):
        self.file = open(file_c_bag)
        self.reader = csv.DictReader(self.file)

    def C_bag_cut(self,c_bag):
        # print(c_bag)
        bag_header=c_bag[12:14]
        bag_content=c_bag[15:29].split('-')
    ...
    def Main_C(self,c_bag):
        str_c_bag=''
        try:
            bag_header,bag_content_list=self.C_bag_cut(c_bag)

            byte_dic=self.byte_dic_tran(bag_header=bag_header)

            str_c_bag +=byte_dic['info']
            ...
            for bag_content in bag_content_list:
                ...
                if type(byte_value) == dict:
                    ...
                    str_c_bag += '[' + count_value_trans + ']'
                elif '/' in byte_value:
                    pass
                elif '2' not in byte_value:
                    byte_value = byte_value + ':' + bag_content_list[n - 1]
                    str_c_bag += '[' + byte_value + ']'

                n +=1
            print('%s翻译：%s'%(c_bag,str_c_bag))
            return str_c_bag
        except:
            error=1
            return error
...
if __name__ == '__main__':
    file_c_bag= r'./Files/C_All.csv'
    c_bag='A5-5A-00-06-00-E1-03-24-00-05-51-1A'
    # c_bag='A5-5A-00-06-07-00-09-00-08-00-4A-AE'
    C_bag_Analyse(file_c_bag=file_c_bag).Main_C(c_bag)
