...
class A_bag_Analyse():
    def __init__(self,file_a_bag):
        self.file = open(file_a_bag)
        self.reader = csv.DictReader(self.file)

    def A_bag_cut(self,a_bag):
        # print(a_bag)
        bag_header=a_bag[12:14]
        bag_content=a_bag[15:29].split('-')
    ...
    def Main_A(self,b_bag,PCHeart=True):
        str_A_bag=''
        try:
            bag_header,bag_content_list=self.A_bag_cut(b_bag)
            byte_dic=self.byte_dic_tran(bag_header=bag_header)

            str_A_bag +=byte_dic['info']
            ...
            for bag_content in bag_content_list:
                ...
                if type(byte_value) == dict:
                    ...
                    str_A_bag += '[' + byte_value + ']'

                n +=1
            print('%s翻译：%s'%(b_bag,str_A_bag))
            return str_A_bag
        except:
            error=1
            return error
...
if __name__ == '__main__':
    file_a_bag= r'./Files/A_All.csv'
    a_bag='AA-55-00-06-81-09-00-08-00-00-94-AF'
    A_bag_Analyse(file_a_bag=file_a_bag).Main_A(a_bag)
